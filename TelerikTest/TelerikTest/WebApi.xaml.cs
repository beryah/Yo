...
namespace TelerikTest
{
    ...
    public sealed partial class WebApi : Page
    {
        ...
        public WebApi()
        {
            ...
            this.httpClient.BaseAddress = new Uri("http://202.3.161.58/ApparelBusinessIntelligenceAPI/");
            //this.httpClient.BaseAddress = new Uri("http://192.168.10.38/ApparelBusinessIntelligenceAPI/");
        ...
        {
            var response = await this.httpClient.GetAsync("api/Values?number=12");
            if (response.IsSuccessStatusCode)
            {
                var jsonString = await response.Content.ReadAsStringAsync();
                this.myTextBox.Text = jsonString;
            }
            else
            {
                this.myTextBox.Text = response.StatusCode.ToString();
            }
        }
        ...
        {
            var response = await this.httpClient.GetAsync("api/Values/15");
            if (response.IsSuccessStatusCode)
            {
                var jsonString = await response.Content.ReadAsStringAsync();
                this.myTextBox.Text = jsonString;
        ...
        private async void Button_Click_3(object sender, RoutedEventArgs e)
        {
            ...
            var js = new DataContractJsonSerializer(typeof(Content));
            var ms = new MemoryStream();
            js.WriteObject(ms, content);
            var sc = new StreamContent(ms);

            var response = await this.httpClient.PostAsync("api/Values", sc);
            if (response.IsSuccessStatusCode)
            {
                this.myTextBox.Text = response.ToString();
            }
            else
            {
                this.myTextBox.Text = response.ToString();
